...
//
// Creates markdown file in psi/memory/learnings/
...
let learningFile = join(ROOT, `psi/memory/learnings/${date}_${slug}.md`);
let count = 1;
while (existsSync(learningFile)) {
  learningFile = join(ROOT, `psi/memory/learnings/${date}_${slug}-${count}.md`);
  count++;
}
