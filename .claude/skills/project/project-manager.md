...
2. **Scan** → Find `psi/memory/*.md`, `learnings/`, `retrospectives/`, `docs/`
3. **Manifest** → Write to `psi/memory/logs/index-YYYY-MM-DD-slug.json`
4. **Log** → Write to `psi/memory/logs/reunion-YYYY-MM-DD.log`
...
- Log to `psi/memory/logs/offload-YYYY-MM-DD.log`
...
- `scripts/learn.sh` - Clone + symlink to psi/learn/
- `scripts/incubate.sh` - Clone (or create) + symlink to psi/incubate/
